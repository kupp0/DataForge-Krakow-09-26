...
CREATE TABLE AttractionRun (
  ...
  Visitors INT64 NOT NULL,
  GrossRevenue NUMERIC AS (TicketPrice * Visitors) STORED,
  CONSTRAINT Chk_TicketPrice CHECK (TicketPrice >= 0),
  CONSTRAINT Chk_Visitors CHECK (Visitors >= 0)
...
CREATE INDEX Idx_AttractionRun_Timestamp 
  ...
  STORING (TicketPrice, Visitors, GrossRevenue);
```
...
> * **NUMERIC & Stored Column**: Financial fields use exact numeric precision, and `GrossRevenue` is automatically calculated and stored at commit time.

### 2. The Economic Pricing Heuristic

Prompt your agent (or run a load-test simulation script) to generate attraction execution runs adhering to the following park constraints:
* **Benchmark Ticket Price**: `$15.00` (Standard turnout: ~80 visitors per run).
...
### 3. Record Sample Executions

Test your table with sample runs:

```sql
INSERT INTO AttractionRun (AttractionID, RunTimestamp, TicketPrice, Visitors) 
VALUES 
  (1, PENDING_COMMIT_TIMESTAMP(), NUMERIC '18.50', 74),
  (7, PENDING_COMMIT_TIMESTAMP(), NUMERIC '22.00', 68),
  (8, PENDING_COMMIT_TIMESTAMP(), NUMERIC '15.00', 80),
  (17, PENDING_COMMIT_TIMESTAMP(), NUMERIC '21.00', 71);
```

### 4. Check the Global Leaderboard

Watch the live dashboard on the event screen to see your city's ranking, total revenue, and awards (e.g. *🏰 Castle Architect*, *💰 Disney Tycoon*, *🔥 Spanner Meltdown*).
