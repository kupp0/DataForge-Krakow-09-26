...
> [!NOTE]
> **Why is there no `Visitors` or `Revenue` column?**  
> Park visitors are not under your direct control! Visitor turnout and ticket sales are governed by the central event market engine according to price elasticity. Your objective as the Theme Park Operator is to determine the optimal **`TicketPrice`** and maximize throughput. The global leaderboard automatically computes your park's attendance and total revenue.

### 2. The Economic Pricing Heuristic

The event leaderboard evaluates your park revenue based on the following market rules:
* **Benchmark Ticket Price**: `$15.00` (Baseline turnout: ~80 visitors per run).
* **The Elasticity Curve**:
  * Increasing ticket prices yields higher margin per head but reduces attendance.
  * Lowering ticket prices packs rides up to maximum capacity (100 visitors) but compresses total revenue.
  * Setting prices over `$35.00` leads to empty queues (*Luxury Trap* badge).
  * Setting prices under `$8.00` leaves money on the table (*Bargain Basement* badge).
  * The mathematical revenue sweet spot is **`$20.00 – $22.50`** per ticket.

### 3. Prompt `agy` in your Workstation VM to Build & Run a Load Test

Switch to your **Cloud Workstation terminal** (in Code-OSS) and launch the **Antigravity CLI (`agy`)** to generate a high-concurrency Spanner load test script:

```bash
agy "Create and execute a high-performance Python load test script for Cloud Spanner that simulates attraction runs. 
The script should:
...
3. In parallel batches of mutations, insert 1,000 AttractionRun records with PENDING_COMMIT_TIMESTAMP().
4. Assign ticket prices to each attraction optimized around the $20.00 - $22.50 sweet spot to maximize park revenue.
5. Log throughput (inserts/sec) and latency so we can stress-test Spanner CPU."
```

Review the implementation plan generated by `agy`, click **Proceed**, and let `agy` execute the load test!

### 4. Monitor Spanner Health & Watch the Global Leaderboard

1. Check your Spanner CPU spike in the Google Cloud Console under **Cloud Spanner > disneyland > Monitoring**.
2. Look at the live projector screen running the **Global Disneyland Leaderboard** to see:
   * Your **City Ranking** (Tokyo, London, Paris, etc.).
   * Your total park **Gross Revenue ($)** calculated live.
   * Your **Spanner CPU %** gauge.
   * Special badges unlocked (*🏰 Castle Architect*, *💰 Disney Tycoon*, *🔥 Spanner Meltdown*).
